...
bool handle_snake_level_key(app_t *app, uint16_t key, uint32_t now) {
    uint8_t max_level = (uint8_t)(ARRAY_COUNT(SNAKE_SPEED_BYTES) - 1u);
    if (key == KEY_UP || key == KEY_2) {
        /* Traced level-screen tones: move 0x03, boundary high 0x08 / low 0x09. */
        if (app->snake_level_draft_byte < max_level) {
            app->snake_level_draft_byte++;
    ...
    if (key == KEY_DOWN || key == KEY_8) {
        if (app->snake_level_draft_byte > 0u) {
            app->snake_level_draft_byte--;
            play_game_system_tone(3u);
        } else {
...
void render_snake_level(const app_t *app, framebuffer_t *fb) {
    ...
    char current[4];
    char max[4];
    snprintf(current, sizeof(current), "%u", (unsigned)(app->snake_level_draft_byte + 1u));
    snprintf(max, sizeof(max), "/%u", (unsigned)ARRAY_COUNT(SNAKE_SPEED_BYTES));
