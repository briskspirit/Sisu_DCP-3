 ...
 * the resulting bench evidence closes the modem electrical bring-up gate.
 */
...
#define DIAG_LTC_WINDOW_MAX_DELAY_S 60u
#define DIAG_LTC_WINDOW_MAX_DURATION_S 3600u
#define DIAG_USB_CUT_MAX_DELAY_S 60u
#define DIAG_SERVICE_VBUS_ATTACH_MS 16u
#define DIAG_SERVICE_VBUS_DETACH_MS 32u
...
typedef struct {
    ...
    int64_t baseline_nah;
    int64_t delta_nah;
    int32_t average_ua;
} diag_ltc_window_t;

static diag_ltc_window_t s_ltc_window;
static bool s_usb_cut_armed;
static uint32_t s_usb_cut_at_ms;
static bool s_usb_phy_attached;
static bool s_usb_vbus_stable;
static bool s_usb_vbus_candidate;
static uint32_t s_usb_vbus_candidate_since_ms;
static bool s_usb_attach_report_pending;
...
static void print_ltc_window(uint32_t timestamp_ms) {
    if (s_ltc_window.armed) {
        int32_t remaining_ms =
            (int32_t)(s_ltc_window.start_at_ms - timestamp_ms);
        if (remaining_ms < 0) {
            remaining_ms = 0;
        }
        ...
        uint32_t elapsed_ms = timestamp_ms - s_ltc_window.started_ms;
...
static void init_usb_vbus_filter(uint32_t timestamp_ms) {
    s_usb_vbus_stable = board_service_vbus_present();
    s_usb_vbus_candidate = s_usb_vbus_stable;
    s_usb_vbus_candidate_since_ms = timestamp_ms;
}

static void poll_usb_attachment(uint32_t timestamp_ms) {
    bool present = board_service_vbus_present();
    if (present != s_usb_vbus_candidate) {
        s_usb_vbus_candidate = present;
        s_usb_vbus_candidate_since_ms = timestamp_ms;
    }
    uint32_t debounce_ms = present ? DIAG_SERVICE_VBUS_ATTACH_MS
                                   : DIAG_SERVICE_VBUS_DETACH_MS;
    if (present != s_usb_vbus_stable &&
        (uint32_t)(timestamp_ms - s_usb_vbus_candidate_since_ms) >=
            debounce_ms) {
        s_usb_vbus_stable = present;
    }

    if (!s_usb_vbus_stable && s_usb_phy_attached) {
        stdio_flush();
        (void)tud_disconnect();
        s_usb_phy_attached = false;
    } else if (s_usb_vbus_stable && !s_usb_phy_attached) {
        (void)tud_connect();
        s_usb_phy_attached = true;
        s_usb_attach_report_pending = true;
    }

    if (s_usb_attach_report_pending && tud_mounted()) {
        s_usb_attach_report_pending = false;
        printf("[usb] physical VBUS restored; CDC reattached\n");
    }
}

static void print_usb_state(void) {
    uint32_t pad = pads_bank0_hw->io[SERVICE_VBUS_PIN];
    printf("[usb] gp%u fn=%d dir=%s out=%u in=%u "
           "ie=%u pue=%u pde=%u iso=%u valid=%u attached=%u | pullup=%u "
           "sie_ctrl=%08lx phy=%08lx override=%08lx\n",
           (unsigned)SERVICE_VBUS_PIN,
           (int)gpio_get_function(SERVICE_VBUS_PIN),
           gpio_is_dir_out(SERVICE_VBUS_PIN) ? "out" : "in",
           gpio_get_out_level(SERVICE_VBUS_PIN) ? 1u : 0u,
           gpio_get(SERVICE_VBUS_PIN) ? 1u : 0u,
           ...
           s_usb_vbus_stable ? 1u : 0u,
           s_usb_phy_attached ? 1u : 0u,
           (usb_hw->sie_ctrl & USB_SIE_CTRL_PULLUP_EN_BITS) ? 1u : 0u,
           (unsigned long)usb_hw->sie_ctrl,
           (unsigned long)usb_hw->phy_direct,
           (unsigned long)usb_hw->phy_direct_override);
}

static void handle_usb_cut(char *cursor, uint32_t timestamp_ms) {
    char *delay_text = next_token(&cursor);
    uint32_t delay_s;
    if (!parse_u32(delay_text, &delay_s) || delay_s == 0u ||
        delay_s > DIAG_USB_CUT_MAX_DELAY_S || !no_more_arguments(cursor)) {
        printf("[usb] usage: usbcut <delay_s 1-60>\n");
        return;
    }

    print_usb_state();
    s_usb_cut_armed = true;
    s_usb_cut_at_ms = timestamp_ms + delay_s * 1000u;
    printf("[usb] D+ pull-up disconnect armed for +%lus; "
           "valid external VBUS will restore CDC\n",
           (unsigned long)delay_s);
}

static void poll_usb_cut(uint32_t timestamp_ms) {
    if (!s_usb_cut_armed ||
        (int32_t)(timestamp_ms - s_usb_cut_at_ms) < 0) {
        return;
    }
    s_usb_cut_armed = false;
    printf("[usb] disabling TinyUSB D+ pull-up now\n");
    stdio_flush();
    (void)tud_disconnect();
    s_usb_phy_attached = false;
}

static void handle_ltc_window(char *cursor, uint32_t timestamp_ms) {
    char *delay_text = next_token(&cursor);
    if (delay_text == NULL) {
        print_ltc_window(timestamp_ms);
        return;
    }
    if (strcmp(delay_text, "cancel") == 0 && no_more_arguments(cursor)) {
        memset(&s_ltc_window, 0, sizeof s_ltc_window);
        printf("[ltcwin] cancelled\n");
        return;
    }

    char *duration_text = next_token(&cursor);
    uint32_t delay_s;
    uint32_t duration_s;
    if (!parse_u32(delay_text, &delay_s) ||
        !parse_u32(duration_text, &duration_s) ||
        delay_s > DIAG_LTC_WINDOW_MAX_DELAY_S || duration_s == 0u ||
        duration_s > DIAG_LTC_WINDOW_MAX_DURATION_S ||
        !no_more_arguments(cursor)) {
        printf("[ltcwin] usage: ltcwin <delay_s 0-60> "
               "<duration_s 1-3600>\n");
        return;
    }
    ...
    s_ltc_window.start_at_ms = timestamp_ms + delay_s * 1000u;
...
static void poll_ltc_window(uint32_t timestamp_ms) {
    if (s_ltc_window.armed &&
        (int32_t)(timestamp_ms - s_ltc_window.start_at_ms) >= 0) {
        ltc2959_snapshot_t ltc;
        ltc2959_hal_get_snapshot(&ltc);
        s_ltc_window.armed = false;
        s_ltc_window.running = true;
        s_ltc_window.started_ms = timestamp_ms;
        s_ltc_window.gauge_session = ltc.gauge_session;
        s_ltc_window.baseline_nah = ltc.session_delta_nah;
    }
    ...
    s_ltc_window.elapsed_ms = timestamp_ms - s_ltc_window.started_ms;
    s_ltc_window.valid =
        ltc.sample_valid && ltc.continuity_valid &&
        ltc.gauge_session == s_ltc_window.gauge_session;
    if (s_ltc_window.valid) {
        s_ltc_window.delta_nah =
            ltc.session_delta_nah - s_ltc_window.baseline_nah;
        s_ltc_window.average_ua = (int32_t)(
            (s_ltc_window.delta_nah * 3600ll) /
            (int64_t)s_ltc_window.elapsed_ms);
    }
}
...
static void print_help(void) {
    ...
    printf("  status\n");
    printf("  ltcwin [<delay_s> <duration_s>|cancel]\n");
    printf("  usbstate\n");
    printf("  usbcut <delay_s 1-60>\n");
...
static void handle_command(char *line, uint32_t timestamp_ms) {
    ...
    if (strcmp(command, "ltcwin") == 0) {
        handle_ltc_window(cursor, timestamp_ms);
        return;
    }
    if (strcmp(command, "usbstate") == 0) {
        if (!no_more_arguments(cursor)) {
            printf("[usb] usage: usbstate\n");
            return;
        }
        print_usb_state();
        return;
    }
    if (strcmp(command, "usbcut") == 0) {
        handle_usb_cut(cursor, timestamp_ms);
        return;
    }
...
int main(void) {
    ...
    while (true) {
        ...
        ltc2959_hal_poll(timestamp_ms, LTC2959_SAMPLE_DEFAULT_PERIOD_MS);
        poll_ltc_window(timestamp_ms);
        poll_usb_cut(timestamp_ms);
        poll_usb_attachment(timestamp_ms);
